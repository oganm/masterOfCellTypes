readDesignMergeCelMAS5 = function (desFile, namingCol, celRegex, celDir,tinyChip, outFolder){
    ...
    newNormalizeds = lapply(affies,mas5)


    
    ned1 = exprs(newNormalizeds[[1]])
    ned2 = exprs(newNormalizeds[[2]])
    ned1[rownames(ned1) %in% rownames(ned2),]
    ned = cbind(ned2,ned1[match(rownames(ned2),rownames(ned1)),])
    ned = log(ned,base=2)
    nprobes <- featureNames(newNormalizeds[[2]])
    nsamp <- c(sampleNames(newNormalizeds[[2]]), sampleNames(newNormalizeds[[1]]))
    ...
    write.csv(aned, paste0(outFolder,"/mas5Exp.csv"), row.names=FALSE)
    ...
    write.table(newDesign, paste0(outFolder,"/meltedDesignMAS5.tsv"), row.names=FALSE,sep = '\t', quote=F)
}
