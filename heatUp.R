...
heatUp = function(expLoc, designLoc, heatFile, heatProps, heatColors, heatPalette, geneList = NA, heig,widt){
    ...
    for (i in 1:len(heatProps)){
        ...
        switch(heatColors[[i]]['method'],
               ...
               gradient = {
                   ...
                   des = as.numeric(as.vector(design[, heatProps[i]]))
                   mi = min(trimNAs(des))
                   ma = max(trimNAs(des))
                   fine = as.numeric(heatColors[[i]]['gradFine'])
                   gradCols = colorRampPalette(c(heatColors[[i]]['lo'], heatColors[[i]]['hi']))(fine)
                   ...
                   precursor = toColor(des[!is.na(des)], palette)
                   temp = rep('white',len(des))
    ...
    for (i in 1:len(allColors)){
        justColors[[i]] = allColors[[i]]$cols
    }
...
# source: https://gist.github.com/nachocab/3853004
# heatmap.3 ----
